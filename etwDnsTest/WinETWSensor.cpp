    ...
    WinETWSensor::                     ~WinETWSensor                        () {
        if (_SessionTraceHandle) {
            ...
            if (ERROR_SUCCESS != status) {
                _logger->log(ERR, "Error %d when stoping event trace.", status);
            }
        }
...
//  ----------------------------------- ----------------------------------- ----------------------------------------------------------------
    ...
    WinETWSensor::                      init                                () {
        ...
        if (NULL == _SessionProperties) {
            _logger->log(ERR, "Unable to allocate %d bytes for properties structure", BufferSize);
            return false;
        }
        ...
        if (ERROR_SUCCESS != status) {
            _logger->log(ERR, "Error %d when starting event trace session", status);
            return false;
        }

        _logger->log(INF, "Event trace session succesfully started");
        ...
        */
        ...
        if (ERROR_SUCCESS != status) {
            _logger->log(ERR, "Error %d when trying to enable provider on event trace session", status);
            _TraceOn = false;
            return false;
        }
        _logger->log(INF, "Event provider succesfully enabled in event trace session.");
        _TraceOn = true;
        return true;
    }
//  ----------------------------------- ----------------------------------- ----------------------------------------------------------------
    void
    WinETWSensor::                      runSensor                           () {
        if (_TraceOn) {
            ...
            trace.EventCallback       = (PEVENT_CALLBACK)(WinETWSensor::ProcessEvent);
            ...
            if (INVALID_PROCESSTRACE_HANDLE == hTrace) {
                _logger->log(ERR, "Error %d when trying to open trace event session to consume events.",GetLastError());
                goto cleanup;
            }
            ...
            if (status != ERROR_SUCCESS && status != ERROR_CANCELLED) {
                _logger->log(ERR, "Error %d when consuming events from trace session.",status);
                goto cleanup;
            }
...
//  ----------------------------------- ----------------------------------- ----------------------------------------------------------------
    ...
    WinETWSensor::                      stopSensor                          () {
        ...
        if (_SessionTraceHandle) {
            ...
            if (ERROR_SUCCESS != status) {
                _logger->log(ERR, "Error %d when stpping event trace session.", status);
            }
        }
...
//  ----------------------------------- ----------------------------------- ----------------------------------------------------------------
    VOID WINAPI
    WinETWSensor::                      ProcessEvent                        (PEVENT_RECORD pEvent) {
        DWORD                           status = ERROR_SUCCESS;
        ...
        // Skips the event if it is the event trace header. Log files contain this event but real-time sessions do not. The event contains the same information as 
        // the EVENT_TRACE_LOGFILE.LogfileHeader member that you can access when you open the trace. 

        /*
        if (IsEqualGUID(pEvent->EventHeader.ProviderId, EventTraceGuid)  &&    pEvent->EventHeader.EventDescriptor.Opcode == EVENT_TRACE_TYPE_INFO) {
            ; // Skip this event.
        }
        else
        */
        {
            ...
            //agent->getSystemLogger()->log(DBG, "Event Time: %02d/%02d/%02d %02d:%02d:%02d.%I64u",             stLocal.wMonth, stLocal.wDay, stLocal.wYear, stLocal.wHour, stLocal.wMinute, stLocal.wSecond, Nanoseconds);
...
//  ----------------------------------- ----------------------------------- ----------------------------------------------------------------
    BOOL
    WinETWSensor::                      getPropertyValue                    (PEVENT_RECORD evt, LPWSTR prop, PBYTE *pData) {
        ...
        *pData = (PBYTE)malloc(PropertySize);
        ...
        if (NULL == *pData) {
            _logger->log(ERR, "Error when trying allocate %d size of memory for property data.", PropertySize);
        ...
        if (ERROR_SUCCESS != status) {
            _logger->log(ERR, "Error %d when trying to get property %s from event.", status, prop);
            if (*pData) {
                free(*pData);
                *pData = NULL;
            }
...
//  ----------------------------------- ----------------------------------- ----------------------------------------------------------------
    DWORD
    WinETWSensor::                      GetEventInformation                 (PEVENT_RECORD pEvent, PTRACE_EVENT_INFO& pInfo) {
        DWORD status = ERROR_SUCCESS;
        DWORD BufferSize = 0;
        ...
        if (ERROR_INSUFFICIENT_BUFFER == status) {
            ...
            if (pInfo == NULL) {
                _logger->log(ERR, "Error when trying allocate %d size of memory for event info.", BufferSize);
                status = ERROR_OUTOFMEMORY;
                goto cleanup;
            }
        ...
        if (ERROR_SUCCESS != status) {
            _logger->log(ERR, "Error %d when trying to get event info.", status);
        }
